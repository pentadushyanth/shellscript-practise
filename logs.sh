...
if [ $? -ne 0 ]; then
    ...
    echo -e "Mysql already exist....$Y Skipping" | tee -a $logfile
fi 
...
if [ $? -ne 0 ]; then
    ...
    echo -e "nginx already exist....$Y Skipping" | tee -a $logfile 
fi 
...
if [ $? -ne 0 ]; then
    ...
    echo -e "python3 already exist....$Y Skipping" | tee -a $logfile
fi 